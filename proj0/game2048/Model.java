...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        boolean isTilted[][] = new boolean[board.size()][board.size()];
        for (int c = 0; c < board.size(); c += 1) {
            for (int r = board.size() - 2; r >= 0; r -= 1) {
                Tile t = board.tile(c, r);
                if (t != null) {
                    int next_r = nextLocal(t, c, r, isTilted);
                    if (board.move(c, next_r, t)){
                        score += board.tile(c, next_r).value();
                        isTilted[c][next_r] = true;
                    }
                    if (next_r != r){
                        changed = true;
                    }
    ...
    public int nextLocal(Tile t, int c, int r, boolean[][] m) {
        int ans;
        ans = board.size() - 1;
        while(ans > r && board.tile(c, ans) != null) {
            ans -= 1;
        }
        if (ans == board.size() - 1) {
            return ans;
        }
        if (board.tile(c, ans + 1).value() == t.value() && !m[c][ans + 1]) {
            return ans + 1;
        }
        return ans;
    }
